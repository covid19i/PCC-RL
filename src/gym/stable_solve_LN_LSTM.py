...
class MyMlpLnLstmPolicy(MlpLnLstmPolicy):
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, n_lstm=lstm_dim, reuse=False, **_kwargs):
        #Instead of layer_norm = True with LstmPolicy, let's try this.
        super(MyMlpLnLstmPolicy, self).__init__(sess, ob_space, ac_space, n_env, n_steps, n_batch, n_lstm, reuse, 
                                          layers = None, net_arch=net_arch, **_kwargs)
        global training_sess
        training_sess = sess
...
model = PPO2(MyMlpLnLstmPolicy, env, verbose=1, nminibatches = 1, n_steps=no_of_timesteps, gamma=gamma)
